...
function parseMatchList(markup) {
  const matches = [];
  const _matchRegex = /\|match\d+=\{\{MatchMaps\n([\s\S]*?)\}\}\s*\n\}/g;
  ...
  for (let i = 1; i < matchBlocks.length; i++) {
    ...
    const maps = [];
    const mapRegex =
      /\|map(\d+)=(\w*)\s*\|map\d+win=(\d*)\s*\|map\d+p1frags=(\w*)\s*\|map\d+p2frags=(\w*)/g;
    let mapMatch;
...
function parseBracket(markup) {
  ...
  const entryRegex =
    /\|R(\d+)([DW])(\d+)=([^|]*)\s*\|R\d+[DW]\d+race=[^|]*\|R\d+[DW]\d+flag=(\w*)\s*\|R\d+[DW]\d+score=(\d*)\s*\|R\d+[DW]\d+win=(\d*)/g;
  let entry;
...
function parseWikiCell(raw) {
  ...
  const attrMatch = trimmed.match(
    /^((?:style|class|colspan|rowspan)\s*=\s*"[^"]*"(?:\s+(?:style|class|colspan|rowspan)\s*=\s*"[^"]*")*)\s*\|\s*(.*)/s
  );
  if (attrMatch) {
...
export function renderStandingsPreview(markup) {
  ...
  if (tables.length === 0)
    return React.createElement('p', { className: 'wiki-empty' }, 'No standings data to preview.');
        ...
        React.createElement(
          ...
          table.info &&
            React.createElement(
              ...
              React.createElement(
                'th',
                { className: 'wiki-table-subtitle', colSpan: 6 },
                table.info
              )
            ),
          // Column headers
        ...
        React.createElement(
          ...
          table.slots.map((slot, si) =>
            React.createElement(
              ...
              React.createElement(
                'td',
                { className: slot.place === 1 ? 'wiki-team-col wiki-team-first' : 'wiki-team-col' },
                React.createElement(
                  'span',
                  { style: { marginRight: '6px' } },
                  countryToFlag(slot.flag)
                ),
                slot.teamName
              ),
              ...
              React.createElement(
                'td',
                {
                  className: slot.diff.startsWith('+')
                    ? 'wiki-stat-diff-pos'
                    : slot.diff.startsWith('-')
                      ? 'wiki-stat-diff-neg'
                      : 'wiki-stat-diff-zero',
                  style: { fontFamily: 'monospace', fontSize: '12px' },
                },
...
export function renderMatchesPreview(markup) {
  ...
  if (matches.length === 0)
    return React.createElement('p', { className: 'wiki-empty' }, 'No match data to preview.');
  ...
  return React.createElement(
    ...
    matches.map((m, i) =>
      React.createElement(
        'div',
        { key: i, className: 'wiki-match-card' },
        // Team 1
        React.createElement(
          'div',
          {
        ...
        React.createElement(
          'div',
          { className: 'wiki-match-score' },
          React.createElement(
            'span',
            { className: m.winner === '1' ? 'wiki-match-score-win' : 'wiki-match-score-lose' },
            m.score1
          ),
          React.createElement('span', { style: { color: '#72777d' } }, ':'),
          React.createElement(
            'span',
            { className: m.winner === '2' ? 'wiki-match-score-win' : 'wiki-match-score-lose' },
            m.score2
          )
        ),
        // Team 2
...
export function renderBracketPreview(markup) {
  ...
  return React.createElement(
    ...
    React.createElement(
      'div',
      { style: { fontSize: '12px', color: '#54595d', marginBottom: '8px' } },
      `${bracketType} Bracket`
    ),
    sortedRounds.map((roundNum) => {
      ...
      return React.createElement(
        ...
        React.createElement(
          'div',
          { className: 'wiki-bracket-round-title' },
        ...
        React.createElement(
          ...
          pairs.map((pair, pi) =>
            React.createElement(
              'div',
              { key: pi, className: 'wiki-bracket-matchup' },
              pair.map((entry, ei) =>
                React.createElement(
                  'div',
                  {
                    ...
                    className: `wiki-bracket-entry ${
                      entry.win
                        ? 'wiki-bracket-entry-winner'
                        : entry.name === 'TBD' || !entry.name
                          ? 'wiki-bracket-entry-tbd'
                          : ''
                    }`,
                  },
                  React.createElement(
                    ...
                    countryToFlag(entry.flag)
                  ),
                  React.createElement(
                    'span',
                    { className: 'wiki-bracket-entry-name' },
                    entry.name || 'TBD'
                  ),
                  React.createElement(
                    'span',
  ...
  return React.createElement(
    ...
    lines.map((line, i) => {
      ...
      if (trimmed.startsWith("'''") && trimmed.endsWith("'''")) {
        return React.createElement(
          'div',
          { key: i, className: 'wiki-text-bold' },
          trimmed.replace(/'''/g, '')
        );
      }
      if (trimmed.startsWith('* ') || trimmed.startsWith('** ')) {
        ...
        return React.createElement(
          'div',
          { key: i, className: 'wiki-text-bullet', style: indent },
          '\u2022 ',
          text
        );
      }
...
export function renderStatsPreview(markup) {
  ...
  if (tables.length === 0)
    return React.createElement('p', { className: 'wiki-empty' }, 'No stats data to preview.');
              ...
              .map((row, ri) =>
                React.createElement(
                  ...
                  row.map((cell, ci) => {
                    ...
                    return React.createElement(
                      'td',
                      { key: ci, style: { ...style, whiteSpace: 'nowrap' } },
                      content
                    );
                  })
                )
...
export function renderWikiPreview(wikiContent, activeExport) {
  ...
  switch (activeExport) {
    ...
    case 'full': {
      ...
      if (
        wikiContent.includes('SEBracket') ||
        wikiContent.includes('=== ') ||
        wikiContent.includes('Winners Bracket')
      ) {
        parts.push(
          React.createElement(
            'div',
    ...
    default:
      return React.createElement(
        'p',
        { className: 'wiki-empty' },
        'Preview not available for this export type.'
      );
  }
}
