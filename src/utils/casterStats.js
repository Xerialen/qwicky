...
import { createTeamContext, resolveTeam } from './teamIdentity';
//
...
/**
 * Normalize team name for comparison — delegates to nameNormalizer.
 * Handles high-bit QW chars, color codes, diacritics, decorators.
 * @param {string} name
 */
export const normalizeTeam = (name) => normalize((name || '').toString());

// ─── Team Name Resolution ────────────────────────────────────────────────────
// All resolution now delegates to teamIdentity.js — the single source of truth.
// The old buildTeamLookup/resolveTeamName functions are replaced by
// createTeamContext/resolveTeam from teamIdentity.

/**
 * Build a lookup context from division teams.
 * @param {Object} division
 * @returns {Object} Team context for use with resolveTeam()
 */
export function buildTeamLookup(division) {
  return createTeamContext(division.teams || []);
}

/**
 * Resolve a ktxstats team name to an admin-entered canonical name.
 * @param {string} jsonTeamName
 * @param {Object} lookup - team context from buildTeamLookup()
 * @returns {string} Canonical team name
 */
export function resolveTeamName(jsonTeamName, lookup) {
  if (!jsonTeamName || !lookup) return jsonTeamName;
  return resolveTeam(jsonTeamName, lookup);
}
...
export function resolveRawMapsTeams(rawMaps, lookup) {
  ...
  return rawMaps.map((m) => {
    const resolvedTeams = (m.teams || []).map((t) => resolveTeam(t, lookup));
    const resolvedScores = {};
    for (const [key, val] of Object.entries(m.scores || {})) {
      resolvedScores[resolveTeam(key, lookup)] = val;
    }
 ...
 * Uses normalize() for key matching (handles high-bit chars, diacritics, etc.).
 * @param {Object} rawMap
...
export const analyzeCommonOpponents = (team1, team2, rawMaps) => {
  ...
  const breakdown = common.map((opponent) => {
    ...
    const advantage =
      Math.abs(diff) >= ADVANTAGE_THRESHOLD ? (diff > 0 ? 'team1' : 'team2') : 'even';
...
export const analyzeRecentForm = (team, rawMaps, lastN = 5) => {
  ...
  const trend =
    secondHalfWins > firstHalfWins + 1
      ? 'rising'
      : secondHalfWins < firstHalfWins - 1
        ? 'falling'
        : 'stable';
...
export const calculatePlayerStats = (rawMaps, lookup) => {
  ...
  for (const p of Object.values(players)) {
    ...
    // K/D and frags/map
    p.kdRatio =
      p.totalDeaths > 0 ? parseFloat((p.totalFrags / p.totalDeaths).toFixed(2)) : p.totalFrags;
    p.fragsPerMap = g > 0 ? parseFloat((p.totalFrags / g).toFixed(1)) : 0;

    // Hot/cold: compare last 3 maps vs tournament average
    const last3 = p.recentFrags.slice(-3);
    const last3Avg =
      last3.length > 0 ? last3.reduce((s, f) => s + f, 0) / last3.length : p.fragsPerMap;
    p.trend =
      last3Avg > p.fragsPerMap * 1.1 ? 'hot' : last3Avg < p.fragsPerMap * 0.9 ? 'cold' : 'steady';
...
export const generateCasterInsights = (team1, team2, rawMaps) => {
  ...
  if (summary.commonCount > 0) {
    ...
    if (breakdown.length >= 2) {
      const t1Std = Math.sqrt(
        breakdown.reduce(
          (s, b) => s + Math.pow(b.team1Result.winRate - summary.team1AvgDominance, 2),
          0
        ) / breakdown.length
      );
      const t2Std = Math.sqrt(
        breakdown.reduce(
          (s, b) => s + Math.pow(b.team2Result.winRate - summary.team2AvgDominance, 2),
          0
        ) / breakdown.length
      );
      if (t1Std < t2Std - 0.15) {
        ...
        insights.push({
          type: 'consistency',
          text: `${team2} are the more consistent side; ${team1}'s form has been more unpredictable.`,
        });
      }
    }
  }
  ...
  if (form1.totalMaps >= 3 && form2.totalMaps >= 3) {
    ...
    if (m1Hot && m2Cold) {
      insights.push({
        type: 'momentum',
        text: `${team1} are riding strong momentum into this match, while ${team2} have struggled recently.`,
      });
    } else if (m2Hot && m1Cold) {
      insights.push({
        type: 'momentum',
        text: `${team2} come in with excellent recent form; ${team1} will be looking to arrest a difficult patch.`,
      });
    } else if (form1.trend === 'rising' && form2.trend === 'falling') {
      insights.push({
        type: 'momentum',
        text: `${team1} are trending upward heading into this match; ${team2} have seen their form dip recently.`,
      });
    } else if (form2.trend === 'rising' && form1.trend === 'falling') {
      insights.push({
        type: 'momentum',
        text: `${team2} are the in-form side right now, with ${team1} looking to turn things around.`,
      });
    }
    ...
    if (form1.streakType === 'W' && form1.streak >= 3) {
      insights.push({
        type: 'momentum',
        text: `${team1} enter on a ${form1.streak}-map winning streak — confidence should be high.`,
      });
    }
    if (form2.streakType === 'W' && form2.streak >= 3) {
      insights.push({
        type: 'momentum',
        text: `${team2} bring a ${form2.streak}-map winning streak into this matchup.`,
      });
    }
  }
  ...
  if (h2h.totalMaps > 0) {
    if (h2h.team1Wins > h2h.team2Wins) {
      insights.push({
        type: 'history',
        text: `${team1} leads the head-to-head ${h2h.team1Wins}–${h2h.team2Wins} in tournament maps played.`,
      });
    } else if (h2h.team2Wins > h2h.team1Wins) {
      insights.push({
        type: 'history',
        text: `${team2} has the historical edge, leading ${h2h.team2Wins}–${h2h.team1Wins} in previous meetings.`,
      });
    } else {
      insights.push({
        type: 'history',
        text: `These teams are perfectly matched: ${h2h.team1Wins} maps each in previous encounters.`,
      });
    }
  } else {
    insights.push({
      type: 'history',
      text: `No prior meetings between these teams in tournament data — this matchup is an open book.`,
    });
  }
