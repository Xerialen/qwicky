      ...
      {player.hasDetailedStats && (
        <div className="border-t border-qw-border/20 pt-2 space-y-1">
          <WeaponRow
            ...
            kills={player.rlKills}
            dropped={player.rlDrop}
          />
          <WeaponRow
            label="LG"
            taken={player.lgTaken}
            kills={player.lgKills}
            dropped={player.lgDrop}
            acc={player.lgAcc}
          />
        </div>
      )}
        ...
        <div className="max-h-56 overflow-y-auto">
          ...
          <div className="space-y-0.5">
            {matches.map((m, i) => (
              <div
                key={i}
                className="grid grid-cols-[1fr_auto_1fr] items-center text-xs font-mono py-0.5"
              >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {teamColumns.map(({ team, players, extMap }) => (
          <div key={team}>
            ...
            >
              {team}
              {isGlobal && <span className="font-normal normal-case"> (+ global)</span>}
            </div>
            <div className="space-y-2">
              {players.length === 0 ? (
...
export default function DivisionCasterView({ division }) {
  ...
  const resolvedMaps = useMemo(
    () => resolveRawMapsTeams(rawMaps, teamLookup),
    [rawMaps, teamLookup]
  );
  ...
  const form1 = useMemo(
    () => (ready ? analyzeRecentForm(team1, resolvedMaps) : null),
    [team1, resolvedMaps, ready]
  );
  const form2 = useMemo(
    () => (ready ? analyzeRecentForm(team2, resolvedMaps) : null),
    [team2, resolvedMaps, ready]
  );
  ...
  const mapStats1 = useMemo(
    () => (ready ? calculateMapStats(team1, resolvedMaps) : null),
    [team1, resolvedMaps, ready]
  );
  const mapStats2 = useMemo(
    () => (ready ? calculateMapStats(team2, resolvedMaps) : null),
    [team2, resolvedMaps, ready]
  );
  ...
  const bracketMatchups = useMemo(() => {
    ...
    const addRound = (round) => {
      ...
      arr.forEach((m) => {
        if (m.team1 && m.team2) matches.push({ team1: m.team1, team2: m.team2, id: m.id });
      });
    };
  ...
  const hasMapData =
    Object.keys(mapStats1 || {}).length > 0 ||
    Object.keys(mapStats2 || {}).length > 0 ||
    extData?.maps1 ||
    extData?.maps2;

  const hasPlayerData =
    (spotlight && spotlight.allPlayers?.length > 0) || extData?.roster1 || extData?.roster2;
  ...
  return (
    <div className="space-y-4">
              ...
              <>
                ...
                <div className="border-t border-qw-border/30 pt-3">
                  <div className="grid grid-cols-[1fr_auto_1fr] items-center text-[10px] font-mono text-qw-muted border-b border-qw-border/20 pb-1 mb-1">
                    ...
                    <span className="text-right truncate" title={team2}>
                      {getTag(team2)}
                    </span>
                  </div>
      ...
      <CollapsibleSection
        title="Recent Form"
        badge={showGlobal ? '🌍 Global 6mo' : '(last 5 maps, oldest → newest)'}
      >
        {showGlobal ? (
          // Global form view
          extLoading ? (
                ...
                <div key={team}>
                  ...
                  {extNorm ? (
                    <div className="space-y-2">
                      ...
                      {localForm &&
                        localForm.wins + localForm.losses > 0 &&
                        (() => {
                          const localPct = Math.round(
                            (localForm.wins / (localForm.wins + localForm.losses)) * 100
                          );
      ...
      {hasMapData && (
        <CollapsibleSection
          title="General Map Performance"
          badge={showGlobal ? '🌍 Global 6mo' : null}
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {[
