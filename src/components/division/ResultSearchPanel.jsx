 ...
 */
export default function ResultSearchPanel({
  division,
  tournament,
  tournamentId,
  onImport,
  ...
  const handleBrowseImport = () => {
    ...
    const rawMaps = selectedGames.map((game) => {
      ...
      let timestamp = null;
      try {
        if (game.playedAt) timestamp = new Date(game.playedAt).getTime();
      } catch {}
      return {
  ...
  const handleDiscoverImport = () => {
    ...
    for (const idx of discoverSelected) {
      ...
      for (const game of series.games) {
        ...
        let timestamp = null;
        try {
          if (game.timestamp) timestamp = new Date(game.timestamp).getTime();
        } catch {}
        rawMaps.push({
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <span
                          className={`text-xs font-semibold px-2 py-0.5 rounded flex-shrink-0 ${isWin ? 'bg-qw-win/20 text-qw-win' : isLoss ? 'bg-qw-loss/20 text-qw-loss' : 'bg-qw-darker text-qw-muted'}`}
                        >
                          {isWin ? 'W' : isLoss ? 'L' : 'D'}
                        </span>
                      </div>
