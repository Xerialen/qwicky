      ...
      <div className="qw-panel overflow-hidden">
        ...
        {!loading && error && (
          <div className="py-12 text-center">
            ...
            <button onClick={load} className="qw-btn text-xs">Retry</button>
          </div>
        )}
        ...
        {!loading && !error && rows.length === 0 && (
          <div className="py-16 text-center">
            ...
            <p className="text-qw-muted text-xs mt-1">Try a longer time period or different mode.</p>
          </div>
        )}
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-qw-dark border-b border-qw-border">
                <th className="w-12 px-4 py-2.5 text-center text-xs font-medium text-qw-muted">#</th>
                <th className="px-4 py-2.5 text-left   text-xs font-medium text-qw-muted">Player</th>
                <th className="w-20 px-4 py-2.5 text-center text-xs font-medium text-qw-muted">Games</th>
                <th className="w-28 px-4 py-2.5 text-right  text-xs font-medium text-qw-accent">{statLabel}</th>
              </tr>
            </thead>
            <tbody>
              {rows.map((row, idx) => {
                ...
                return (
                  ...
                  >
                    <td className="px-4 py-2 text-center font-mono text-xs text-qw-muted">
                      {isFirst ? (
                        <span className="text-qw-accent font-bold">1</span>
                      ) : (
                        idx + 1
                      )}
                    </td>
                    <td className="px-4 py-2 font-medium text-sm text-qw-text">
                      {player}
                    </td>
                    <td className="px-4 py-2 text-center font-mono text-xs text-qw-muted">
                      {games}
